...
7. Select Minimum SDK API 35 ("VanillaIceCream") (Choosing a higher level will result in fewer phones to choose from, but we'll be using an emulator.) 
8. Select the Build configuration languae Gradle DSL, then click Finish. 
9. The Gradle build will now occur - it can take some time (the status bar is at the bottom of the window); wait for it to load everything.
...
**(Optional)** If the Gradle Build errors out, check File->Project Structure -> Project. The Gradle Version should be 8.11. For me, even though these seemed to be set correctly, I had to reset them before it would work. Then the Gradle build executed again, and all was good.
...
3. Click the green triangle. The Gradle build will take a few moments if you've changed any settings.
4. Switch to the running AVD (the second icon in the icon menu on the right hand side) and verify that the Hello Android app has successfully launched. Note that it may take a few moments for the emulator's phone to boot up. The emulator should start as a separate program (it may be behind other windows). It should say "First Fragment" with a Next Button, along with some garbage text.
...
There are some controls above the phone emulator, including the on/off switch. You may have to turn the phone on.
...
On the left side, expand the Project view. You should see a more complicated directory structure than the usual IntelliJ listing, becaaus there's more stuff to keep track of.
